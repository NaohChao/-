...
uint8 TX_ADDRESS[5] = {0x12, 0x34, 0x06, 0x63, 0x81};   // 定义一个静态发送地址
uint8 RX_ADDRESS[5] = {0x12, 0x34, 0x06, 0x63, 0x81};

#define CHANAL          25                              //频道选择
...
uint32  nrf_rx(uint8 *rxbuf, uint32 len)
{
    ...
    while( (nrf_rx_flag != QUEUE_EMPTY) && (len != 0) )
    {
        if(len < DATA_PACKET)
        {
            fire_cpy(rxbuf, (uint8 *)&(NRF_ISR_RX_FIFO[nrf_rx_front]), len);
        ...
        fire_cpy(rxbuf, (uint8 *)&(NRF_ISR_RX_FIFO[nrf_rx_front]), DATA_PACKET);
        rxbuf   += DATA_PACKET;
        len     -= DATA_PACKET;
        tmplen  += DATA_PACKET;
